...
(function () {
    ...
    async function syncVentas() {
        ...
        const url = new URL(<?php echo json_encode(route('ventas.sync'), 15, 512) ?>.replace(/&amp;/g, '&'), window.location.origin);
        ...
        try {
            ...
            tableBody.innerHTML = j.rows.map(row => {
                const stocks = <?php echo json_encode($sedesStock, 15, 512) ?>.map(sede => `<td>${row.stocks[sede] ?? 0}</td>`).join('');
