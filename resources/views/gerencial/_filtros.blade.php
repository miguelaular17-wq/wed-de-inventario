...
<form method="GET" action="{{ $action }}" class="nomina-card gerencial-filtros">
    <div class="nomina-form-grid">
        ...
        @if(in_array($modo, ['completo', 'devoluciones', 'rentabilidad', 'clientes'], true))
            <div class="field gerencial-vendedor-field">
                ...
                @php
                    ...
                    $vendedorActual = $vendedoresSel[0] ?? '';
                    $vendedorActualClave = $gerencialVendSvc->claveVendedor($vendedorActual);
                    $listaVendedores = collect($catalogos['vendedores'] ?? [])
                        ->map(fn ($v) => (string) $v)
                        ->unique()
                        ->sort(fn ($a, $b) => strcmp(mb_strtoupper($a, 'UTF-8'), mb_strtoupper($b, 'UTF-8')))
                        ->values();
                @endphp
                <select name="vendedor">
                    ...
                    @foreach($listaVendedores as $vend)
                        @php
                            $sel = $vendedorActual !== '' && (
                                mb_strtoupper($vend, 'UTF-8') === mb_strtoupper($vendedorActual, 'UTF-8')
                                || $gerencialVendSvc->claveVendedor($vend) === $vendedorActualClave
                            );
                        @endphp
                        <option value="{{ $vend }}" @selected($sel)>{{ $vend }}</option>
                    @endforeach
                </select>
            </div>
        @endif
