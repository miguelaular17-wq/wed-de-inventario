...
class UserController extends Controller
{
    ...
    public function update(Request $request, User $user)
    {
        $data = $request->validate([
            'role' => ['required', 'string', 'in:admin,supervisor,telefonia,comprador,sede,vendedor,marketing,finanzas,cobranza,contabilidad,auditor,tesoreria'],
            'sede' => ['nullable', 'string'],
            'password_plain' => ['nullable', 'string', 'min:6'],
        ]);

        $user->role = $data['role'];
        if (in_array($data['role'], ['comprador', 'marketing', 'finanzas', 'cobranza', 'contabilidad', 'auditor', 'tesoreria'], true)) {
            $user->sede = null;
        } else {
